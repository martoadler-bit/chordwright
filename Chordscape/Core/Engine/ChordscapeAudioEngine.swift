...
/// since Chordscape's signal path is fixed (no patch graph to coordinate, unlike
/// Modula).
///
...
/// audio thread.
final class ChordscapeAudioEngine: ObservableObject {
    static let stepCount = 16

    @Published var currentChordLabel: String = "–"
    @Published var currentStep: Int = 0
    @Published var stepHasContent: [Bool] = Array(repeating: false, count: ChordscapeAudioEngine.stepCount)
    ...
    private var loopSteps: [[Int]?] = Array(repeating: nil, count: ChordscapeAudioEngine.stepCount)
    ...
    func start() {
        ...
        do {
            ...
            print("Chordscape audio engine failed to start: \(error)")
        }
    }
    ...
    /// engine), not a single shared timbre. Caller (`ChordscapeViewModel`)
