...
/// forwards Note On/Off to `ChordwrightViewModel`, which snaps each incoming
...
final class MIDIManager: ObservableObject {
    ...
    private weak var viewModel: ChordwrightViewModel?
    private var client = MIDIClientRef()
    private var inputPort = MIDIPortRef()

    init(viewModel: ChordwrightViewModel) {
        self.viewModel = viewModel
        setUpCoreMIDI()
    }

    private func setUpCoreMIDI() {
        let clientStatus = MIDIClientCreateWithBlock("ChordwrightMIDIClient" as CFString, &client) { [weak self] _ in
            Task { @MainActor in
        ...
        let portStatus = MIDIInputPortCreateWithBlock(client, "ChordwrightInputPort" as CFString, &inputPort) { [weak self] packetList, _ in
