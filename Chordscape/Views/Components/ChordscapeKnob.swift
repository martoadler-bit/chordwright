...
/// — Chordscape's controls are simple enough for a plain range). Vertical drag
/// changes value; double-tap resets to `defaultValue`.
struct ChordscapeKnob: View {
    let label: String
    let value: Float
    ...
    var body: some View {
        VStack(spacing: 4) {
            ZStack {
                Circle()
                    .fill(Color.chordscapePanel)
                    .overlay(Circle().stroke(Color.chordscapeInk.opacity(0.12), lineWidth: 1))
                Circle()
                    .trim(from: 0, to: CGFloat(normalized))
                    .rotation(.degrees(-135))
                    .stroke(Color.chordscapeCoral, style: StrokeStyle(lineWidth: 4, lineCap: .round))
                    .padding(5)
                Rectangle()
                    .fill(Color.chordscapeInk)
            ...
            Text(label)
                ...
                .foregroundStyle(Color.chordscapeInk.opacity(0.7))
                .lineLimit(1)
                .minimumScaleFactor(0.7)
                .frame(maxWidth: diameter + 16)
            Text(valueText(value))
                .font(.system(size: 10, design: .monospaced))
                .foregroundStyle(Color.chordscapeInk.opacity(0.45))
        }
    }
}
