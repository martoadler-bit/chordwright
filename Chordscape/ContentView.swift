...
struct ContentView: View {
    @StateObject private var viewModel: ChordscapeViewModel
    @ObservedObject private var engine: ChordscapeAudioEngine
    ...
    @AppStorage("chordscape.isDarkMode") private var isDarkMode = false

    init() {
        let vm = ChordscapeViewModel()
    ...
    private let padColors: [Color] = [
        .chordscapeCoral, .chordscapeGold, .chordscapeMoss, .chordscapeMint, .chordscapeSky, .chordscapeLavender, .chordscapeBlush
    ]

    var body: some View {
        ZStack {
            Color.chordscapeBackground.ignoresSafeArea()
            ...
            // long/tall on both iPad and iPhone. A trailing `Spacer()`
            // takes over the "soak up leftover space" job instead, so any
            // extra room on a tall screen goes below the pads as breathing
            // room rather than stretching them.
    ...
    private var screen: some View {
        ...
        .frame(maxWidth: .infinity)
        .background(RoundedRectangle(cornerRadius: 16, style: .continuous).fill(Color.chordscapePanel))
    }
    ...
    private var padRow: some View {
        GeometryReader { geo in
            ...
            let padWidth = (geo.size.width - spacing * CGFloat(ChordscapeViewModel.degreeCount - 1)) / CGFloat(ChordscapeViewModel.degreeCount)
            ZStack {
                HStack(spacing: spacing) {
                    ForEach(0..<ChordscapeViewModel.degreeCount, id: \.self) { i in
                        PadView(
    ...
    private func transportButton(icon: String, active: Bool, activeColor: Color = .chordscapeCoral, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Image(systemName: icon)
                .font(.system(size: 15, weight: .semibold))
                .frame(width: 42, height: 42)
                .background(Circle().fill(active ? activeColor : Color.chordscapePanel))
                .foregroundStyle(active ? Color.white : Color.chordscapeInk)
        }
        .buttonStyle(.plain)
    }
}
